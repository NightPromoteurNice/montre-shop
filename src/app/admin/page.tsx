...
export default function Admin() {
  ...
  const handleSubmit = async () => {
    ...
    if (image) {
      ...
      const { data, error } = await supabase.storage.from('watches').upload(filename, image, { contentType: image.type })
      if (!error && data) {
    ...
    const { error } = await supabase.from('watches').insert([{
      ...
      description: form.description, category: form.category, image_url, stock: parseInt(form.stock)
    }])
    setLoading(false)
    if (!error) {
      setSuccess(true)
