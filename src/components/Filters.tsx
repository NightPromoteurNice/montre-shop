...
export default function Filters({ filters, onChange, isOpen, onClose, isMobile = false }: Props) {
      ...
      <div className="mb-8 border-b border-white/5 pb-8">
        ...
        <div className="flex gap-3 items-center">
          <input
            ...
            onChange={e => onChange({ ...filters, minPrice: parseInt(e.target.value) || 0 })}
            placeholder="Min"
          ...
          <input
            ...
            onChange={e => onChange({ ...filters, maxPrice: parseInt(e.target.value) || 999999 })}
            placeholder="Max"
            className="w-full bg-white/5 border border-white/10 px-3 py-2 text-sm focus:outline-none focus:border-white/30 text-white placeholder:text-white/20"
          />
        </div>
        ...
        <div className="flex flex-col gap-2">
          {BRANDS.map(brand => (
            <button key={brand} onClick={() => toggleBrand(brand)}
              className={`text-left text-sm py-1 transition-colors flex items-center gap-3 ${
  ...
  if (isMobile) {
    return (
      <>
        {isOpen && <div className="fixed inset-0 bg-black/60 z-30" onClick={onClose} />}
        <div className={`fixed top-0 left-0 h-full w-72 bg-[#0a0a0a] border-r border-white/10 z-40 overflow-y-auto transition-transform duration-300 ${
